...
func WebHandler(response http.ResponseWriter, request *http.Request) {
	...
	if len(modifiedPath) > 0 && modifiedPath[0] == '/' {
		modifiedPath = modifiedPath[1:]
	}
	lastChar := len(modifiedPath) - 1
	if lastChar >= 0 && modifiedPath[lastChar] == '/' {
		modifiedPath = modifiedPath[:lastChar]
	}
	...
	for _, queryType := range possibleQueryTypes {
		...
		case "block":
			...
			if request.Method != "GET" {
				fmt.Println(fmt.Sprintf("%s must be sent as a GET request.", queryType))
				break
			}
		...
		case "block-tx":
			...
			if paramCount < 3 {
				fmt.Println("No id provided for tx. Request ignored.")
				break
			}
			if len(params[1]) != 64 {
				fmt.Println(fmt.Sprintf("%s is not a valid tx id", params[1]))
				break
			}

			blockIndex, err := strconv.Atoi(params[2])
			if err != nil {
				fmt.Println(fmt.Sprintf("block index (%s) not formatted correctly, error: %s", params[2], err.Error()))
				break
			}
		...
		case "input":
			...
			if request.Method != "POST" {
				fmt.Println(fmt.Sprintf("%s must be sent as a POST request.", queryType))
				break
			}
...
func getBlockHtml(block btc.Block, customJavascript string) string {
	...
	if len(previousHash) > 0 {
		blockHtmlData["PreviousHash"] = previousHash
	}
	nextHash := block.GetNextHash()
	if len(nextHash) > 0 {
		blockHtmlData["NextHash"] = nextHash
	}
...
type BlockChartData struct {
	NonCoinbaseInputCount uint16            `json:"non_coinbase_input_count"`
	OutputCount           uint16            `json:"output_count"`
	SpendTypes            map[string]uint16 `json:"spend_types"`
	OutputTypes           map[string]uint16 `json:"output_types"`
}
...
func getBlockTxResponse(tx btc.Tx, blockIndex uint16) BlockTxResponse {
	...
	var buff bytes.Buffer
	if err := templ.ExecuteTemplate(&buff, "BlockTx", blockTxData); err != nil {
		panic(err)
	}
...
func getInputHtml(htmlData InputHtmlData) string {
	...
	var buff bytes.Buffer
	if err := templ.ExecuteTemplate(&buff, "InputDetail", htmlData); err != nil {
		panic(err)
	}
...
func getInputHtmlData(input btc.Input, txIndex uint16, satoshis uint64, bip141 bool) InputHtmlData {
	...
	if len(previousOutputType) > 0 {
		...
		if len(htmlData.PreviousOutputAddress) == 0 {
			htmlData.PreviousOutputAddress = "No Address Format"
		}
	}
...
func extractBodyFromHTML(html string) string {
	...
	if bodyBegin > -1 {
		...
		fmt.Println("Body not found in chart HTML.")
	}
...
func getBlockCharts(nonCoinbaseInputCount uint16, outputCount uint16, spendTypes map[string]uint16, outputTypes map[string]uint16) map[string]string {
	...
	for _, typeName := range spendTypeNames {
		if spendTypes[typeName] > 0 {
			if len(typeName) > longestLabel {
				longestLabel = len(typeName)
			}
