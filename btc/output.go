...
func NewOutput(value uint64, script Script, address string) Output {
	...
	outputType := ""
	if script.IsTaprootOutput() {
		outputType = OUTPUT_TYPE_TAPROOT
	} else if script.IsP2wpkhOutput() {
		outputType = OUTPUT_TYPE_P2WPKH
	} else if script.IsP2wshOutput() {
		outputType = OUTPUT_TYPE_P2WSH
	} else if script.IsP2shOutput() {
		outputType = OUTPUT_TYPE_P2SH
	} else if script.IsP2pkhOutput() {
		outputType = OUTPUT_TYPE_P2PKH
	} else if script.IsMultiSigOutput() {
		outputType = OUTPUT_TYPE_MultiSig
	} else if script.IsP2pkOutput() {
		outputType = OUTPUT_TYPE_P2PK
	} else if script.IsNullDataOutput() {
		outputType = OUTPUT_TYPE_OP_RETURN
	} else if script.IsWitnessUnknownOutput() {
		outputType = OUTPUT_TYPE_WitnessUnknown
	} else {
