...
func NewScript(rawBytes []byte) Script {
	...
	if fieldCount > 0 {
		...
		for f, field := range fieldMap {
			...
			if fieldText == "OP_IF" || fieldText == "OP_NOTIF" {
				ifCount++
			}
			if fieldText == "OP_ELSE" {
				elseCount++
			}
			if fieldText == "OP_ENDIF" {
				endIfCount++
			}
			if fieldText == "OP_RETURN" {
				opReturnCount++
			}
			fields[f] = field
		}
...
func (s *Script) SetFieldType(fieldIndex int, fieldType string) {
	...
	if fieldCount <= fieldIndex {
		fmt.Println("Setting type of field ", fieldIndex, " in script that only has ", fieldCount, " fields.")
		return
	}
...
func (s *Script) IsMultiSigOutput() bool {
	...
	for i := sigCountIndex + 1; i < pubKeyCountIndex; i++ {
		if !IsValidECPublicKey(s.fields[i].AsBytes()) {
			return false
		}
	}
	...
	if isValidOpcode(pubKeyCount) && pubKeyCount >= 0x51 && pubKeyCount <= 0x60 {
		pubKeyCount -= 0x50
	}
	if int(pubKeyCount) > fieldCount-3 {
		return false
	}

	// verify the number of signatures
	sigCount := s.fields[sigCountIndex].rawBytes[0]
	if isValidOpcode(sigCount) && sigCount >= 0x51 && sigCount <= 0x60 {
		sigCount -= 0x50
	}
...
func (s *Script) IsNonstandardOutput() bool {
	return !s.IsTaprootOutput() && !s.IsP2wpkhOutput() && !s.IsP2wshOutput() && !s.IsP2shOutput() && !s.IsP2pkhOutput() && !s.IsMultiSigOutput() && !s.IsP2pkOutput() && !s.IsNullDataOutput() && !s.IsWitnessUnknownOutput()
}
...
func (s *Script) IsWitnessUnknownOutput() bool {
	...
	if !firstByteIsValidWitnessVersion {
		return false
	}
...
func (s *Script) IsOrdinal() bool {
	...
	if !IsValidSchnorrPublicKey(s.fields[0].AsBytes()) {
		return false
	}
	if s.fields[1].AsHex() != "OP_CHECKSIG" {
		return false
	}

	ordBegin := 2
	if s.fields[3].AsHex() == "OP_DROP" {
		ordBegin = 4
	}

	if s.fields[ordBegin].AsHex() != "OP_0" {
		return false
	}
	if s.fields[ordBegin+1].AsHex() != "OP_IF" {
		return false
	}
	if string(s.fields[ordBegin+2].AsBytes()) != "ord" {
		return false
	}
	if s.fields[ordBegin+3].AsBytes()[0] != 0x01 {
		return false
	}

	if s.fields[ordBegin+5].AsHex() != "OP_0" {
		return false
	}

	if s.fields[fieldCount-1].AsHex() != "OP_ENDIF" {
		return false
	}
